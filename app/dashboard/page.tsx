...
import { syncProfileDailyStreak } from '@/lib/profile';
...
export default function DashboardPage() {
  ...
  const isReady = !loading && !!session && !!profile && localXp !== null;
  ...
  useEffect(() => {
    if (!userId || !profile || localXp === null) return;
    ...
    const checkAchievements = async () => {
      ...
      try {
        ...
        const perfectDaySet = new Set(
          (dailyLogs ?? [])
            .filter((log: { webdev_completed: boolean; jog_completed: boolean }) => log.webdev_completed && log.jog_completed)
            .map((log: { date: string }) => Number(log.date.split('-')[2]))
        );

        let longestStreak = 0;
        let run = 0;
        for (let day = 1; day <= daysInMonth; day++) {
          if (perfectDaySet.has(day)) {
            run++;
        ...
        const unlocked = await unlockMissingAchievements(supabase, userId, {
          completedQuests: (quests ?? []).filter((quest: { is_completed: boolean }) => quest.is_completed).length,
          currentStreak,
          longestStreak,
          level: levelInfo.level,
          rank: getRankDetails(levelInfo.level).rank,
          sleepConsistencyDays: (sleepLogs ?? []).filter((log: { hours: number }) => Number(log.hours) >= 7).length,
        });
  ...
  if (error?.code) {
    ...
    checkAchievements();
  }, [achievementRefreshKey, localXp, profile, userId, supabase]);

  const handleXpChange = useCallback(
    async (xpChange: number) => {
      if (!userId || localXp === null) return;
    ...
    [userId, localXp, refreshProfile]
  );

  const handlePerfectDay = useCallback((dayNum: number) => {
    setPerfectDayNum(dayNum);
    setPerfectDayActive(true);
    setAchievementRefreshKey((key) => key + 1);
  }, []);

  const handleDailyLogChange = useCallback(async () => {
    if (!userId) return;

    try {
      await syncProfileDailyStreak(supabase, userId);
      await refreshProfile();
    } catch (error) {
  ...
  const handleResetProgress = async () => {
    ...
    if (
      ...
      try {
        ...
        const { error: sleepError } = await supabase.from('sleep_logs').delete().eq('user_id', userId);
  ...
  if (!isReady || !session || !profile || localXp === null) {
    return (
    ...
    <div className="flex-1 w-full max-w-4xl mx-auto px-4 py-8 flex flex-col gap-6 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          />
          <QuestList userId={session.user.id} onXpChange={handleXpChange} />
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <DailyLog
            ...
            onLogChange={handleDailyLogChange}
          />
        </div>
      </div>
